...
Gemini Integration: Utilizes the gemini-2.5-flash model for high-speed, cost-effective reasoning.

## How It Works
The agent operates in a loop:
Analyze: The LLM determines the next best step based on the user's prompt and current state.
Act: The agent calls a predefined Python function (e.g., write_file or run_python_file).
Observe: The output of the function is fed back into the LLM.
Repeat: The process continues until the agent provides a final response confirming the task is complete.
